...
/**
 * Stream stdout of `srcCmd` (on `src`) into stdin of `dstCmd` (on `dst`). Used
 * to relay build artifacts Mac -> Linux without buffering them on the API host:
 * the bytes flow through `ssh2`'s duplex channels back-to-back, with Node's
 * Readable.pipe() handling backpressure automatically. Memory footprint is
 * constant regardless of file size.
 *
 * Resolves with the byte count once *both* channels close cleanly. Rejects if
 * either side exits non-zero or errors — partial uploads are caller's problem
 * to clean up.
 */
export function pipeBetweenSsh(
  src: Client,
  srcCmd: string,
  dst: Client,
  dstCmd: string,
  ...
  return new Promise((resolve, reject) => {
    src.exec(srcCmd, { pty: false }, (err, srcStream) => {
      if (err) return reject(err);
      dst.exec(dstCmd, { pty: false }, (err2, dstStream) => {
        if (err2) {
          try { srcStream.destroy(); } catch { /* noop */ }
          return reject(err2);
        }
        let bytes = 0;
        let srcErr = "";
        let dstErr = "";
        let srcExit: number | null = null;
        let dstExit: number | null = null;
        let settled = false;
        const finish = (e?: Error) => {
          if (settled) return;
          if (e) { settled = true; return reject(e); }
          if (srcExit === null || dstExit === null) return; // wait for both
          settled = true;
          if (srcExit !== 0) return reject(new Error(`source cmd exited ${srcExit}: ${srcErr.trim().slice(0, 300)}`));
          if (dstExit !== 0) return reject(new Error(`dest cmd exited ${dstExit}: ${dstErr.trim().slice(0, 300)}`));
          resolve({ bytes });
        };
        ...
        srcStream.stderr.on("data", (c: Buffer) => { srcErr += c.toString("utf8"); });
        srcStream.on("error", (e: Error) => finish(e));
        srcStream.on("close", (code: number | null) => { srcExit = code ?? -1; finish(); });

        dstStream.on("data", () => { /* drain */ });
        dstStream.stderr.on("data", (c: Buffer) => { dstErr += c.toString("utf8"); });
        dstStream.on("error", (e: Error) => finish(e));
        dstStream.on("close", (code: number | null) => { dstExit = code ?? -1; finish(); });

        // pipe() ends dstStream's writable side when srcStream emits 'end', which
        // is exactly what we need to flush `cat > file` on the Linux side.
...
/** Tiny helper: read a file's size via `stat`. Caller picks the format flag (-c %s on Linux, -f %z on macOS). */
export function statSize(client: Client, path: string, fmtFlag: string): Promise<number> {
  return new Promise((resolve, reject) => {
    client.exec(`stat ${fmtFlag} '${path.replace(/'/g, `'\\''`)}'`, { pty: false }, (err, stream) => {
      if (err) return reject(err);
      let out = ""; let errBuf = "";
      stream.on("data", (c: Buffer) => { out += c.toString("utf8"); });
      stream.stderr.on("data", (c: Buffer) => { errBuf += c.toString("utf8"); });
      stream.on("close", (code: number | null) => {
        if (code !== 0) return reject(new Error(`stat exited ${code}: ${errBuf.trim().slice(0, 200)}`));
        const n = parseInt(out.trim(), 10);
        if (!Number.isFinite(n)) return reject(new Error(`unparseable size: ${out.trim().slice(0, 80)}`));
        resolve(n);
      });
    });
  });
